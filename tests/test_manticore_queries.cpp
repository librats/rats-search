...
private:
    ...
    Torrent makeTorrent(int n, const QString& name, ContentType type = ContentType::Video,
                        qint64 size = 1024 * 1024, int seeders = 10);
...
Torrent TestManticoreQueries::makeTorrent(int n, const QString& name, ContentType type,
                                          qint64 size, int seeders)
{
    Torrent t;
...
void TestManticoreQueries::testSelectQueryAgainstLiveIndex()
{
    ...
    const QString sql = SelectQuery("torrents").columns("id, hash, name")
                            .whereRaw("seeders > 0")
