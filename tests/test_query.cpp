...
{
    const QString jsonData = QString::fromUtf8(
        R"({"files":[{"path":"1C/Кашаев -- 'Обучение в 1С' -- 2012.pdf","size":1144147}]})");
    QVariantList params;
...
{
    QCOMPARE(SelectQuery("torrents").columns("id, hash").build(),
             QString("SELECT id, hash FROM torrents"));
}
...
void TestQuery::testSelect_whereIn_empty()
{
    ...
    QCOMPARE(SelectQuery("torrents").whereIn("hash", {}).build(),
             QString("SELECT * FROM torrents WHERE 1 = 0"));
}

void TestQuery::testSelect_whereRaw()
{
    QCOMPARE(SelectQuery("torrents").whereRaw("size > 100").build(),
             QString("SELECT * FROM torrents WHERE size > 100"));
    // Empty raw condition is ignored.
    QCOMPARE(SelectQuery("torrents").whereRaw("").build(),
             QString("SELECT * FROM torrents"));
}
...
void TestQuery::testSelect_orderBy_rejectsInjection()
{
    ...
    const QString sql = SelectQuery("torrents")
                            .orderBy("seeders; DROP TABLE torrents", true)
                            .build();
