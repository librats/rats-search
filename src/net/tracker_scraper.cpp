...
void TrackerScraper::requestScrape(const QString& infoHash, const QStringList& trackers)
{
    ...
    // Deduplicate against the per-hash cooldown, and opportunistically prune stale
    // entries so recentChecks_ stays bounded regardless of uptime.
    {
...
int TrackerScraper::pendingCount() const
{
    QMutexLocker locker(&queueMutex_);
    return pendingQueue_.size();
}

QStringList TrackerScraper::defaultTrackers()
{
