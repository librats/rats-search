...
// The authoritative strategy list. Its size — not a hardcoded constant — is what
// every scrape uses as its pending-result count.
...
bool TrackerInfoScraper::wasRecentlyScraped(const QString& infoHash) const
{
    QMutexLocker locker(&recentChecksMutex_);
    if (!recentChecks_.contains(infoHash)) {
        return false;
    }
    return recentChecks_[infoHash].secsTo(QDateTime::currentDateTime()) < cooldownSecs_;
}
