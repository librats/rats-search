...
//
// Borrows the librats BitTorrent subsystem from a P2PTransport (non-owning) and:
...
public:
    ...
    // Monitoring
    int discoveredCount() const; // torrents whose metadata was fetched
    int pendingCount() const; // info-hashes queued for a metadata fetch
    size_t dhtNodeCount() const; // DHT routing-table size
    size_t spiderPoolSize() const; // spider node pool (separate from routing table)
    size_t spiderVisitedCount() const;

    // Tuning
    void setWalkInterval(int intervalMs);
    int walkInterval() const;
    void setIgnoreInterval(int intervalMs);
    int ignoreInterval() const;
    void setMetadataFetchEnabled(bool enabled);
    bool metadataFetchEnabled() const;

    // Predicate consulted for each freshly seen info-hash before a BEP 9 metadata
    // fetch is queued: returning true skips the fetch (e.g. the torrent is already
    // indexed). Keeps the crawler DB-free — the caller injects the lookup. Called
    // on the Qt thread (onAnnounce is already marshalled there).
...
signals:
    ...
    // Emitted the first time a new info-hash is seen on the DHT (lightweight).
    void hashDiscovered(const QString& infoHash);
    // Emitted once metadata has been fetched and a full torrent model built.
    void discovered(const rats::domain::Torrent& torrent);
    void discoveredCountChanged(int count);
...
private:
    // A pending metadata fetch. Replaces the old "hash|ip|port" packed string.
