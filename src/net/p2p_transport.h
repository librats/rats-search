...
public:
    ...
    QStringList connectedPeerIds() const;
...
signals:
    ...
    void messageReceived(const QString& peerId, const QString& type, const QJsonObject& data);
...
private:
    ...
    // this takes effect on the next (re)start. Kept as a direct member because the
    // inline portMappingEnabled() getter needs it while Private is still opaque.
