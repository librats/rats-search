...
// completed) for a given info-hash. Pure transport-side helper: it only talks to
// trackers and emits results — it never touches the database. A higher-level
...
//   - a per-hash cooldown (kDefaultCheckIntervalSecs) so the same torrent is not
...
public:
    ...
    // Number of requests currently waiting in the overflow queue.
    int pendingCount() const;

    // Built-in list of well-known public trackers used when none are supplied.
...
private:
    ...
    // Drop cooldown entries older than the cooldown window so recentChecks_ cannot
    // grow without bound over long uptimes. Caller must hold recentChecksMutex_.
