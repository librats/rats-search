...
// the scraped() signal as a JSON object whose keys match what the old code
// merged into the torrent's `info` field (poster, description, contentCategory,
// trackers[], rutrackerThreadId, nyaaThreadId, trackerName). A higher-level
...
public:
    ...
    bool wasRecentlyScraped(const QString& infoHash) const;
