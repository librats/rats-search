...
Crawler::Crawler(P2PTransport* transport, QObject* parent)
    ...
    , ignoreIntervalMs_(DEFAULT_IGNORE_INTERVAL_MS)
...
int Crawler::discoveredCount() const
{
    return discoveredCount_.load();
}

int Crawler::pendingCount() const
{
    return pendingCount_.load();
}

size_t Crawler::dhtNodeCount() const
{
    if (!transport_) {
        return 0;
    }
    return transport_->dhtNodeCount();
}

size_t Crawler::spiderPoolSize() const
{
#ifdef RATS_SEARCH_FEATURES
    librats::Bittorrent* bt = bittorrent();
    if (bt) {
...
size_t Crawler::spiderVisitedCount() const
{
#ifdef RATS_SEARCH_FEATURES
    librats::Bittorrent* bt = bittorrent();
    if (bt) {
...
int Crawler::walkInterval() const
{
    return walkIntervalMs_;
}

void Crawler::setIgnoreInterval(int intervalMs)
{
...
int Crawler::ignoreInterval() const
{
    return ignoreIntervalMs_;
}

void Crawler::setMetadataFetchEnabled(bool enabled)
{
...
bool Crawler::metadataFetchEnabled() const
{
    return metadataFetchEnabled_;
}
...
void Crawler::onAnnounce(const std::array<uint8_t, 20>& infoHash, const std::string& ip, uint16_t port)
{
    ...
    emit hashDiscovered(hashHex);

    // Skip torrents we already have: fetching BEP 9 metadata for them would burn a
    // fetch slot and bandwidth only for the insert path to discard the result as a
    // duplicate. The caller injects the "already indexed?" lookup to keep the
    // crawler DB-free (restores the old spider's pre-fetch database->hasTorrent()).
...
void Crawler::onMetadataReceived(const rats::domain::Torrent& torrent)
{
    ...
    discoveredCount_++;
    emit discoveredCountChanged(discoveredCount_.load());
