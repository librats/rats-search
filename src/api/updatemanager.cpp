...
bool UpdateManager::createUpdateScript(const QString& updateDir)
{
    ...
    out << ")\r\n";
    out << "\r\n";
    out << "echo Waiting for searchd to exit...\r\n";
    out << ":waitsearchd\r\n";
    out << "tasklist /FI \"IMAGENAME eq searchd.exe\" 2>NUL | find /I \"searchd.exe\" >NUL\r\n";
    out << "if not errorlevel 1 (\r\n";
    out << "    echo searchd.exe is still running, attempting to stop...\r\n";
    out << "    taskkill /F /IM searchd.exe >nul 2>&1\r\n";
    out << "    timeout /t 2 /nobreak >nul\r\n";
    out << "    tasklist /FI \"IMAGENAME eq searchd.exe\" 2>NUL | find /I \"searchd.exe\" >NUL\r\n";
    out << "    if not errorlevel 1 (\r\n";
    out << "        timeout /t 1 /nobreak >nul\r\n";
    out << "        goto waitsearchd\r\n";
    out << "    )\r\n";
    out << ")\r\n";
    out << "\r\n";
    ...
    out << "done\n";
    out << "\n";
    out << "# Wait for searchd to exit (it runs as a separate process)\n";
    out << "if pgrep -x 'searchd' > /dev/null; then\n";
    out << "    echo 'Waiting for searchd to exit...'\n";
    out << "    kill $(pgrep -x 'searchd') 2>/dev/null\n";
    out << "    for i in $(seq 1 10); do\n";
    out << "        pgrep -x 'searchd' > /dev/null || break\n";
    out << "        sleep 1\n";
    out << "    done\n";
    out << "    # Force kill if still running\n";
    out << "    if pgrep -x 'searchd' > /dev/null; then\n";
    out << "        kill -9 $(pgrep -x 'searchd') 2>/dev/null\n";
    out << "        sleep 1\n";
    out << "    fi\n";
    out << "fi\n";
    out << "\n";
