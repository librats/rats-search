...
// REST/WebSocket/P2P response envelope, so services, the API router and the peer
// API all speak the same shape and there is a single fail-vs-empty policy.
class Result {
public:
    Result() = default;
...
// once, on the main thread (see MainThread::post). This removes the ad-hoc mix
// of callbacks, QtConcurrent and manual invokeMethod in the old RatsAPI.
