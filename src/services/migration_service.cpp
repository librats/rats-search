...
void MigrationService::registerMigrations()
{
    ...
    //
    // Fields: {id, minVersion, maxVersion, description, isSync, syncBody, asyncBody}
    //   - minVersion / maxVersion: inclusive app-version bounds (empty = no bound)
...
bool MigrationService::syncUpdateWalkInterval()
{
    ...
    // Bump to the new default (100ms) if still on the old aggressive value.
        ...
        QMetaObject::invokeMethod(
            ...
            [this]() {
                ...
                if (!stopRequested_.load()) {
                    ...
                    qInfo() << "[Migration] async migrations stopped (will resume on next startup)";
                    saveState();
                }
            },
            Qt::QueuedConnection);
