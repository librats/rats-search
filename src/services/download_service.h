...
// seeding). This is the former ActiveTorrent, now owned by the service layer.
struct Download {
    QString hash;
    QString name;
    QString savePath;
    ...
    // Rolling sample used to derive downloadSpeed from the cumulative byte counter
    // between polls (librats exposes total bytes, not a per-torrent rate).
...
public:
    ...
    // Add by known metadata (hash + optional name/size), fetching content via DHT.
...
signals:
    ...
    // The single progress signal (consolidated from the old duplicate shapes).
    ...
    void pauseStateChanged(const QString& hash, bool paused);
    void stateChanged(const QString& hash, const QJsonObject& state);
    void torrentCreated(const QString& hash, const QString& name, qint64 size);
...
private:
    ...
    // Small de-duplicated helpers (each formerly copy-pasted several times).
