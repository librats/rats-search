...
// Stored/wire shape: the torrent JSON (with files, for P2P replication) plus the
// extra feedDate. Info is intentionally omitted to keep the feed payload lean.
...
int FeedService::maxSize() const
{
    return maxSize_;
}

void FeedService::setMaxSize(int max)
{
    maxSize_ = max;
    bool trimmed = false;
...
void FeedService::add(const FeedItem& item)
{
    ...
    markDirty();
    emit itemAdded(item.torrent.hash);
    emit feedUpdated();
}
...
QVector<FeedItem> FeedService::allItems() const
{
    return feed_;
}
...
bool FeedService::contains(const QString& hash) const
{
    return index_.contains(hash);
}

FeedItem FeedService::getItem(const QString& hash) const
{
    const auto it = index_.constFind(hash);
    if (it == index_.constEnd())
        return FeedItem();
    return feed_[it.value()];
}
