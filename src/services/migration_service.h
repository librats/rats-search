...
// directory.
//
// This is the rats::service port of the old top-level MigrationManager. The
// behaviour, migration ids and on-disk state format are preserved verbatim so an
// existing migrations.json still resolves. What changed is the internals:
...
//   - migrations operate on the new data layer (data::Database /
//     data::TorrentRepository) and reclassify via domain::ContentClassifier
//     rather than the old TorrentDatabase god-class.
