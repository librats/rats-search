...
// source (DHT crawler, P2P replication, .torrent import, feed sync). Replaces
// the old RatsAPI::processAndInsertTorrent, which every source called through a
// tangle of const_casts and cross-thread emits. The flow is: validate ->
// dedupe (merging incoming votes) -> classify -> filter -> persist -> notify.
