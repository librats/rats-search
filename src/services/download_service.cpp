...
DownloadService::~DownloadService()
{
    ...
    // Persist resume data for every active download so downloaded pieces survive a
...
bool DownloadService::resume(const QString& hash)
{
    ...
    engine_->resume(h);
    emit pauseStateChanged(h, false);
...
QString DownloadService::registerSeed(const net::SeedResult& seed)
{
    ...
    emit torrentCreated(hash, d.name, d.totalSize);
    return hash;
}
...
QString DownloadService::defaultDownloadPath() const
{
