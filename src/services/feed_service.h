...
// A single entry in the voted-torrents feed: a domain torrent plus the moment it
// entered the feed. `feedDate` drives the recency half of the ranking and is the
// only field the feed adds on top of the plain torrent — everything else lives on
// the torrent itself (votes, seeders, files), so the codec serialises the torrent
// and only `feedDate` is stitched in alongside.
struct FeedItem {
    domain::Torrent torrent;
    qint64 feedDate = 0;
};

// The ranked, in-memory voted-torrents feed. Items are ordered by a recency+votes
// score (see calculateScore) and capped at maxSize. This is the port of the old
// api/FeedManager onto the layered architecture: persistence is delegated to a
// data::FeedRepository and addByHash lookups to a data::TorrentRepository.
//
// Persistence is debounced. The old FeedManager rewrote the entire feed table on
// every single add() (a delete-all + full reinsert, O(n) DB churn per vote); here
// mutations only mark the feed dirty and a short coalescing timer flushes one full
// rewrite. save() forces an immediate flush and the destructor flushes if dirty.
...
public:
    ...
    bool save();
    // Empty the feed (persisted on the next flush).
    void clear();
    ...
    QVector<FeedItem> allItems() const;
    ...
    FeedItem getItem(const QString& hash) const;
