...
// Windows daemon mode: grace period for the parent QProcess to reach NotRunning.
...
// If the QMYSQL driver is missing, how long to wait when tearing the process down.
...
{
    // Check if an external instance is running via the PID file (fast check first).
...
void Manticore::stop()
{
...
#ifdef Q_OS_WIN
    // On Windows, searchd runs as a daemon, so we need to use the stopwait command
    ...
    // Clean up the original QProcess object to prevent a "destroyed while running" warning.
    // In Windows daemon mode, the parent process has already exited, but QProcess
    // may not have properly detected this. We need to ensure it's in the NotRunning state.
    if (process_) {
        if (process_->state() != QProcess::NotRunning) {
            // Try to wait for the process to finish (should be instant since the parent already exited).
...
{
    // Each thread needs its own connection - include the thread ID in the connection name
    QString threadConnName
        = connectionName_ + "_" + QString::number(reinterpret_cast<quintptr>(QThread::currentThread()));
...
bool Manticore::waitForReady(int timeoutMs)
{
    ...
    while (timer.elapsed() < timeoutMs) {
        ...
        if (!isWindowsDaemonMode_) {
            ...
            // On Windows, check if the PID file was created as a sign of startup progress.
...
#ifdef Q_OS_WIN
    // On Windows, searchd forks into the background and the parent exits with code 0.
