...
// file list. Centralises the parse that was duplicated in the old
// getFilesForTorrent / getFilesForTorrents.
...
void TorrentRepository::primeFromDatabase()
{
    ...
    const auto rows = db_->query(
        QStringLiteral("SELECT COUNT(*) AS cnt, SUM(files) AS numfiles, SUM(size) AS totalsize FROM torrents"));
    if (!rows.isEmpty()) {
...
QVector<SearchHit> TorrentRepository::searchFiles(const SearchQuery& q)
{
    ...
    const QString sql = QStringLiteral("SELECT *, SNIPPET(path, ?, 'around=100', 'force_all_words=1') AS snipplet FROM "
    ...
    for (const auto& row : db_->query(inSql)) {
        ...
        // Populate a lightweight file list from the matched paths, as the old UI expects.
...
QHash<QString, QVector<File>> TorrentRepository::filesOf(const QStringList& hashes)
{
    ...
    // whereIn() escapes every hash — the old code hand-quoted them.
