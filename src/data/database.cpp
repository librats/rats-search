...
Database::Rows Database::query(const QString& sql, const QVariantList& params, bool* ok)
{
    ...
    if (!db.isOpen() && !db.open()) {
        ...
        emit queryError(err);
        qWarning() << "Database: connection not open:" << err << "| SQL:" << sql.left(200);
        return rows;
    }
    ...
    if (!q.exec(finalSql)) {
        ...
        emit queryError(err);
        qWarning() << "Database: query failed:" << err << "| SQL:" << finalSql.left(200);
        return rows;
    }
...
bool Database::runWrite(const QString& sqlText, const char* op, const QString& table)
{
    ...
    if (!db.isOpen() && !db.open()) {
        const QString err = db.lastError().text();
        emit queryError(err);
        qWarning() << "Database:" << op << "connection not open:" << err;
        return false;
    }
    ...
    if (!q.exec(sqlText)) {
        const QString err = q.lastError().text();
        emit queryError(err);
        qWarning() << "Database:" << op << "failed:" << err << "| SQL:" << sqlText.left(200);
        return false;
    }
...
bool Database::optimize(const QString& table)
{
    return execute(QStringLiteral("OPTIMIZE INDEX %1").arg(table));
}

bool Database::flushRamchunk(const QString& table)
{
    return execute(QStringLiteral("FLUSH RAMCHUNK %1").arg(table));
}
