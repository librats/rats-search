...
// Persistence for the voted-torrents feed. The whole feed lives in the Manticore
// `feed` table (schema: rt_field feedIndex, rt_attr_json data) as one JSON blob
// per row, exactly as the old FeedManager stored it. The repository is
// deliberately dumb about feed semantics: it moves opaque JSON objects in and out
// of the table, so the item shape (ranking, dates) stays a service concern and
// the data layer never depends on the service layer.
