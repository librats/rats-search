...
// backslash notation and spliced into raw SQL. This module is the ONE place that
// does that escaping — the old codebase had several call sites that bypassed it
// (hand-quoted IN clauses, a raw ORDER BY %1 injection vector, unescaped MATCH
// input). Everything now goes through here.
...
// are deliberately left intact so prefix search keeps working. The result is
// still a plain string to be handed to formatValue()/quote().
...
// starts with a letter/underscore, then letters/digits/underscores only. Used to
// validate ORDER BY columns before they are spliced into SQL.
...
// string values go through the escaping above. ORDER BY columns are validated as
// identifiers, closing the old `ORDER BY %1` injection hole.
class SelectQuery {
public:
