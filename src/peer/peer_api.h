...
// The P2P half of the old RatsAPI, extracted into its own front-end. It answers
// other peers' requests (search / files / top / torrent / feed / randomTorrents)
// by running local service queries and replying over the transport, and it
// processes their responses by funnelling every received torrent through the one
// IndexingService insertion path — never touching the database directly.
//
// Wire message-type names are preserved verbatim so already-deployed peers keep
// interoperating. The client_info handshake is deliberately NOT handled here: it
// lives in PeerRegistry. On connect this only drives the follow-up work the old
// handleP2PPeerConnected did on top of the handshake (feed + replication pull).
//
...
signals:
    ...
    // the old API never echoed it back — and torrents is the raw wire array with
    ...
    // A remote peer answered a topTorrents request (topTorrents_response).
    ...
    // A remote peer's feed was accepted and merged (feed_response).
    void feedReceived(const QJsonArray& feed);
    // A genuinely new torrent was indexed from a peer (any response path).
    void torrentReplicated(const QString& hash, const QString& name);
...
private:
    ...
    void handleTopTorrentsResponse(const QString& peerId, const QJsonObject& data);
    ...
    // Insert one received torrent through IndexingService (the single write path).
