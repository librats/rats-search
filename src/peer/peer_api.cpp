...
void PeerApi::install()
{
    ...
    transport->registerHandler("topTorrents_response",
        [this](const QString& peerId, const QJsonObject& data) { handleTopTorrentsResponse(peerId, data); });
    ...
    // Follow-up work on connect (the client_info handshake itself is PeerRegistry's).
...
void PeerApi::handleTorrentRequest(const QString& peerId, const QJsonObject& data)
{
    ...
    if (!torrent || !torrent->isValid())
        return; // not found: stay silent, matching the old behaviour
...
void PeerApi::handleFeedRequest(const QString& peerId, const QJsonObject& data)
{
    ...
    // FeedService serialises each item with its file list already, so no
    // per-item enrichment (the old N+1 fix) is needed here.
...
void PeerApi::handleTopTorrentsResponse(const QString& peerId, const QJsonObject& data)
{
    const QString type = data["type"].toString();
    const QString time = data["time"].toString();

    QJsonArray torrents;
    for (const QJsonValue& v : data["torrents"].toArray()) {
    ...
    qInfo() << "[PeerApi] received" << torrents.size() << "top torrents from" << shortId(peerId);
    emit remoteTopTorrents(torrents, type, time);
}
...
void PeerApi::handleFeedResponse(const QString& peerId, const QJsonObject& data)
{
    ...
    // Replicate the torrents behind the feed into the index (no replication
    // accounting — the old feed path did not count these).
    for (const QJsonValue& v : remoteFeed) {
        if (v.isObject())
            insertFromPeer(v.toObject(), /*trackReplication*/ false);
    }

    emit feedReceived(remoteFeed);
}
...
bool PeerApi::insertFromPeer(const QJsonObject& data, bool trackReplication)
{
    ...
    emit torrentReplicated(result.torrent.hash, result.torrent.name);
    return true;
}
