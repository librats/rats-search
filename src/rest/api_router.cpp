...
// insert and the response happen there. This is the single async source-combine
// that used to be copy-pasted twice in RatsAPI (search + getTorrent).
...
void ApiRouter::wireEvents()
{
    ...
    // through direct signal connections. Event names/payload shapes match the
    // old RatsAPI/ApiServer contract for client compatibility.
    ...
    connect(app_->indexing(), &service::IndexingService::torrentIndexed, this, [this](const domain::Torrent& torrent) {
        emit event(QStringLiteral("torrentIndexed"), QJsonObject { { "hash", torrent.hash }, { "name", torrent.name } });
    });
    connect(app_->voting(), &service::VotingService::votesUpdated, this, [this](const QString& hash, int good, int bad) {
        emit event(QStringLiteral("votesUpdated"), QJsonObject { { "hash", hash }, { "good", good }, { "bad", bad } });
    });
    connect(app_->feed(), &service::FeedService::feedUpdated, this,
        [this]() { emit event(QStringLiteral("feedUpdated"), QJsonObject { { "feed", app_->feed()->toJsonArray() } }); });
...
void ApiRouter::registerMethods()
{
    ...
    // torrents that no longer pass (e.g. after tightening the adult/size filters).
    // With dryRun=true it only counts; otherwise it removes and emits progress.
