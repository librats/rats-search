...
//
// Every request is translated into a single router->call(method, params, respond)
// dispatch; the Result handed back is serialized into the legacy response
// envelope. Push events emitted by ApiRouter::event() are broadcast to all
// connected WebSocket clients (download progress, feed updates, config changes).
//
// This replaces the old src/api/apiserver.* which routed through RatsAPI and
// wired each broadcast signal by hand; here there is a single event channel.
...
public:
    ...
    //   wsPort:   port for WebSocket (0 disables, -1 shares by using httpPort + 1).
