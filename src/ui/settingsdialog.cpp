...
QWidget* SettingsDialog::createStorageTab()
{
    ...
    // NOTE: The old dialog wired the RatsAPI::cleanupProgress signal here and the
    // buttons below called RatsAPI::removeTorrents(). The new layered architecture
    // has no equivalent service (no bulk filter-based torrent cleanup / progress
    // reporting is exposed on Application), so the cleanup action is currently
    // unavailable. The UI is kept intact; the slots report the unavailability.
...
void SettingsDialog::runCleanup(bool dryRun)
{
    ...
    QApplication::processEvents(); // paint the status before the synchronous sweep
