...
void TorrentDetailsPanel::setTorrent(const rats::domain::Torrent& torrent)
{
    ...
    const QString typeName = (torrent.contentType == ContentType::Unknown)
        ? tr("Unknown")
        : capitalizeFirst(rats::domain::toString(torrent.contentType));
    ...
    categoryLabel_->setText(typeUnknown ? tr("Unknown")
                                        : capitalizeFirst(rats::domain::toString(torrent.contentType))
                + (catUnknown ? QString()
                              : " (" + capitalizeFirst(rats::domain::toString(torrent.contentCategory)) + ")"));
    ...
    // Kick off background tracker (counts + website info) refresh via the service.
...
void TorrentDetailsPanel::onFavoriteClicked()
{
    ...
    if (fav->isFavorite(currentHash_)) {
        ...
        emit addToFavoritesRequested(currentHash_);
    }
...
// the row and refreshes stats + scraped info. Rate limiting lives in the service.
// ============================================================================
