...
void MainWindow::connectSignals()
{
    ...
    if (app_->config()) {
        ...
        connect(app_->config(), &rats::app::ConfigStore::languageChanged, this, &MainWindow::onLanguageChanged);
    }
}
...
void MainWindow::connectServiceSignals()
{
    ...
    // Download progress mirrored into the details panel if it is showing that torrent.
    if (app_->downloads()) {
    ...
    if (app_->exporter()) {
        ...
        connect(exporter, &rats::service::TorrentExporter::exportFailed, this,
            [this](const QString&, const QString& reason) {
                QMessageBox::warning(this, tr("Export Torrent"),
                    tr("Could not export the torrent.\n\n%1").arg(reason));
            });
...
void MainWindow::closeEvent(QCloseEvent* event)
{
    ...
    // Persist UI state; service shutdown (DB, P2P, session) is main()/Application's job.
    saveSettings();
    event->accept();
...
void MainWindow::onTabChanged(int index)
{
    ...
    if (currentWidget == resultsTableView->parentWidget()) {
        ...
        // Any of the torrent-table / activity tabs: restore its remembered selection.
...
void MainWindow::onDownloadRequested(const QString& hash)
{
    ...
    if (app_->downloads()) {
        ...
        if (ok) {
            ...
            QMessageBox::warning(
                this, tr("Download Failed"), tr("Could not start the download. Check the info hash and try again."));
        }
    }
}
...
void MainWindow::onLanguageChanged(const QString& languageCode)
{
    ...
    if (app_->translation())
        app_->translation()->setLanguage(languageCode);
}
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (dialog.needsRestart()) {
            QMessageBox::information(this, tr("Restart Required"),
                tr("Some changes (network ports or data directory) will take effect after restarting the "
                   "application."));
        }
        saveSettings();
    }
...
{
    if (app_->updates() && app_->updates()->isUpdateAvailable()) {
        const auto& info = app_->updates()->updateInfo();
...
bool MainWindow::showAgreementDialog()
{
    ...
    QLabel* subtitleLabel
        = new QLabel(tr("Please read and accept the following End User License Agreement before using this software."));
...
**IF YOU DO NOT AGREE TO THESE TERMS, DO NOT USE THIS SOFTWARE.**
    ...
    QLabel* warningLabel = new QLabel(
        tr("⚠️ By clicking 'I Accept', you acknowledge that you have read, understood, and agree to be bound by all "
           "terms and conditions of this Agreement. You accept full responsibility for your use of this Software."));
