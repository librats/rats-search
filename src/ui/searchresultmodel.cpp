...
void SearchResultModel::addResults(const QVector<SearchHit>& results)
{
    QVector<SearchHit> newResults;
...
{
    if (row >= 0 && row < results_.size()) {
        return results_[row].torrent.hash;
    }
    return QString();
}
...
void SearchResultModel::setFileResults(const QVector<SearchHit>& results)
{
    // File results are merged with existing torrent results
    // If a torrent already exists, merge the matching paths
    for (const SearchHit& fileResult : results) {
...
void SearchResultModel::clearFileResults()
{
    ...
    QVector<SearchHit> torrentOnlyResults;
    for (SearchHit& hit : results_) {
        if (!hit.fromFileMatch) {
            // Keep torrent search results, but clear any merged file paths
            hit.matchingPaths.clear();
            torrentOnlyResults.append(hit);
        }
...
{
    int count = 0;
    for (const SearchHit& result : results_) {
        if (!result.fromFileMatch || !result.matchingPaths.isEmpty()) {
            // Count if it's a torrent match or has matching files
            if (!result.fromFileMatch) {
                count++;
            }
        }
    }
