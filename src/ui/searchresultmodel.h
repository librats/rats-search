 ...
 * Rows are stored as rats::domain::SearchHit (torrent + how it was matched).
 */
...
public:
    ...
    void addResults(const QVector<rats::domain::SearchHit>& results);
    void clearResults();

    // File search results methods
    void setFileResults(const QVector<rats::domain::SearchHit>& results);
