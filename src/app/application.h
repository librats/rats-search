...
// Composition root: owns every adapter and service and wires them together in
// dependency order. Replaces the three roles the old god-objects fused — service
// ownership (MainWindow), subsystem wiring (RatsAPI::initialize) and lifecycle.
//
// Front-ends (GUI window, REST/WS router, console shell, P2P peer API) receive a
// running Application and only call into its services through the accessors
