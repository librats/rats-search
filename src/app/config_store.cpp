...
// (udpTrackersTimeout, restApi, cleanup, spider.*, ...) have no entry and fall
...
void ConfigStore::setDefaults()
{
    config_ = QJsonObject { // Network
        { "httpPort", 8095 }, { "p2pPort", 4444 }, { "dhtPort", 6881 }, { "udpTrackersTimeout", 180000 },
        ...
        // Spider
        { "spider", QJsonObject { { "walkInterval", 100 }, { "nodesUsage", 100 }, { "packagesLimit", 500 } } },
        ...
        // Cleanup
        { "cleanup", true }, { "cleanupDiscLimit", 7LL * 1024 * 1024 * 1024 }, { "spaceQuota", false },
        { "spaceDiskLimit", 7LL * 1024 * 1024 * 1024 }, { "recheckFilesOnAdding", true },
...
int ConfigStore::udpTrackersTimeout() const
{
    return config_["udpTrackersTimeout"].toInt(180000);
}
void ConfigStore::setUdpTrackersTimeout(int timeout)
{
    setValue("udpTrackersTimeout", timeout);
}
...
void ConfigStore::setUpnpEnabled(bool enabled)
{
    setValue("upnp", enabled);
}
...
int ConfigStore::spiderNodesUsage() const
{
    return config_["spider"].toObject()["nodesUsage"].toInt(100);
}
void ConfigStore::setSpiderNodesUsage(int usage)
{
    setValue("spider.nodesUsage", usage);
}

int ConfigStore::spiderPackagesLimit() const
{
    return config_["spider"].toObject()["packagesLimit"].toInt(500);
}
void ConfigStore::setSpiderPackagesLimit(int limit)
{
    setValue("spider.packagesLimit", limit);
}
...
bool ConfigStore::cleanupEnabled() const
{
    return config_["cleanup"].toBool(true);
}
void ConfigStore::setCleanupEnabled(bool enabled)
{
    setValue("cleanup", enabled);
}

qint64 ConfigStore::cleanupDiscLimit() const
{
    return config_["cleanupDiscLimit"].toVariant().toLongLong();
}
void ConfigStore::setCleanupDiscLimit(qint64 limit)
{
    setValue("cleanupDiscLimit", QVariant(static_cast<qlonglong>(limit)));
}

bool ConfigStore::spaceQuotaEnabled() const
{
    return config_["spaceQuota"].toBool(false);
}
void ConfigStore::setSpaceQuotaEnabled(bool enabled)
{
    setValue("spaceQuota", enabled);
}

qint64 ConfigStore::spaceDiskLimit() const
{
    return config_["spaceDiskLimit"].toVariant().toLongLong();
}
void ConfigStore::setSpaceDiskLimit(qint64 limit)
{
    setValue("spaceDiskLimit", QVariant(static_cast<qlonglong>(limit)));
}

bool ConfigStore::recheckFilesOnAdding() const
{
    return config_["recheckFilesOnAdding"].toBool(true);
}
void ConfigStore::setRecheckFilesOnAdding(bool enabled)
{
    setValue("recheckFilesOnAdding", enabled);
}
