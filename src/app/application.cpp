...
void Application::applyConfig()
{
    ...
    // where start() itself does the initial launch). start()/stop() are idempotent.
    if (d_->running) {
...
bool Application::start()
{
    ...
    // Blocking pre-start migrations run before anything reads the data.
    d_->migrations->runSyncMigrations();
